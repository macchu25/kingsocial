    ...
    try {
      ...
      if (response.success) {
        ...
        setReels((prev) =>
          prev.map((r) =>
            r.id === reel.id
              ? { ...r, isLiked: response.isLiked, likes: response.likes }
              : r
          )
        );
      } else {
  ...
  const handleFollow = async (reel) => {
    ...
    try {
      ...
      if (response.success) {
        setIsFollowing((prev) => ({
          ...prev,
          [reel.userId]: response.isFollowing,
        }));
  ...
  const renderReel = useCallback(
    ({ item: reel, index }) => {
      ...
      const handleVideoPress = () => {
        ...
        if (
          lastTap.current[reelId] &&
          now - lastTap.current[reelId] < DOUBLE_PRESS_DELAY
        ) {
        ...
        <View style={styles.reelContainer}>
          ...
          <View style={styles.bottomInfo}>
            <View style={styles.userSection}>
              <TouchableOpacity
                onPress={() =>
                  onViewUserProfile(reel.userId, reel.username, reel.userAvatar)
                }
