...
export const postService = {
  ...
  // Tạo post mới
  createPost: async (images, caption, type = 'post', isSensitive = false, sensitiveReason = null) => {
    const headers = await getAuthHeaders();
    const baseUrl = getBaseUrl();
    // Support both single image (backward compatibility) and multiple images
    const payload = Array.isArray(images) 
      ? { images, caption, type, isSensitive, sensitiveReason }
      : { image: images, caption, type, isSensitive, sensitiveReason };
